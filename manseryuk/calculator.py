...
def gan_to_hanja(korean_text):
    return ''.join(Gan_to_hanja.get(char, char) for char in korean_text)


def ji_to_hanja(korean_text):
    return ''.join(Ji_to_hanja.get(char, char) for char in korean_text)
...
def determine_zodiac_hour_str(hour_str, minute_str):
    ...
    # 12지지에 따른 시간 범위 설정
    time_ranges = [
        ((23*60 + 30, 23*60 + 59), "익일"),  # 23:30 ~ 23:59 (자정까지)
        ((0, 1*60 + 29), "자"),       # 00:00 ~ 01:29
        ((1*60 + 30, 3*60 + 29), "축"),   # 01:30 ~ 03:29
        ((3*60 + 30, 5*60 + 29), "인"),   # 03:30 ~ 05:29
        ((5*60 + 30, 7*60 + 29), "묘"),   # 05:30 ~ 07:29
        ((7*60 + 30, 9*60 + 29), "진"),   # 07:30 ~ 09:29
        ((9*60 + 30, 11*60 + 29), "사"),  # 09:30 ~ 11:29
        ((11*60 + 30, 13*60 + 29), "오"), # 11:30 ~ 13:29
        ((13*60 + 30, 15*60 + 29), "미"), # 13:30 ~ 15:29
        ((15*60 + 30, 17*60 + 29), "신"), # 15:30 ~ 17:29
        ((17*60 + 30, 19*60 + 29), "유"), # 17:30 ~ 19:29
        ((19*60 + 30, 21*60 + 29), "술"), # 19:30 ~ 21:29
        ((21*60 + 30, 23*60 + 29), "해")  # 21:30 ~ 23:29
    ]
